---
title: "College Football Bowl Projections 2017-2018"
...
games over the course of the season against other FBS (Football Bowl Subdivision) schools; given that there 
are over 120 FBS teams in the NCAA, that means that each team will only play approximately 10% of all other teams that they will be compared to.  The very nature of this has lead to much controversy over the years, and makes it difficult to determine which teams are better than others simply based on win and loss totals.
...
First, we'll install (if necessary) and load the packages required:

```{r, message = FALSE, warning = 'hide'}
...
football scores from every week in 2017 from this site and combine it into one dataframe.
...
Note that current structure of the data.  All of the basic information is there (i.e. home team, score, date, etc) but that it is not in a suitable format for analysis.  
...
The first major step is to clean and normalize the data.
...
Because the data is stored on a third party website, it is inconsiderate (at best) to continually read and load data directly from the site.  As such, it is prudent to save/load a local image of the data at this point to use.

Saving/Loading the workspace, as needed:

```{r, eval = FALSE}

save("C:/Users/Bryan/Google Drive/CFB2017/data2017.RData")
...
Basic feature generation; separating game data (such as score, location, number of overtimes, etc) into multiple fields.  
...
Determining the winning and losing teams and encoding those results to factor variables
...
Because of the way that aggregating functions work, it is necessary to recombine all of the results with themselves in order to more easily obtain aggregate totals.  For example, one record will capture the following information:  

Team A scored 42 points, Team B scored 14 points, Team A was the home team, Team A was the winning team and Team B was the losing team.  

However, when generating aggregate team data, it is easier to have all teams and all games represented in each feature field, so the code below will the state the above data in a slightly different way to represent the same result but in a functionally different way:

Team B scored 14 points, Team A scored 42 points, Team B was NOT the home team, Team B was the losing team and Team A was the winning team.

This subtle distinction is also important when in comes to projecting bowl games.  A significant majority of regular season games are played with the paradigm of one team playing at home and one team playing away.  However, bowl games don't follow that same rule, so it's important to allow a model to distinguish between "playing at another team's home field" and "not playing at home".
...
Continuing the process of cleaning the data and separating it into feature fields
...
Games that go into overtime are rare but have a tendency to skew final game scores to be much higher than games that end in regulation.  Because variance in score increases as the number of individual scores (touchdowns and field goals) increase, lower scoring games tend to end in ties more often than higher scoring games.  As such, scores in overtime games need to be normalized to a total that better reflects how the game actually played out.
...
Feature engineering.  Simply going by wins, losses, and scores against opponents without any indication of how that opponent normally plays against other teams doesn't provide much insight.  Features need to be created that give more information about how good each opponent actually is; these features include average points scored, average points scored against, winning percentage, average margin of victory, etc.  These team stats are normalized and unusable data will be imputed with average values from all other teams.
...
teamstats <- teamstats %>% mutate(normalteamscore = pnorm(as.numeric(team.avg.point), 
                                                          mean = mean(teamstats$team.avg.point), 
                                                          sd = sd(teamstats$team.avg.point)),
                                  normaloppscore = pnorm(as.numeric(opp.avg.point), 
                                                          mean = mean(teamstats$opp.avg.point), 
                                                          sd = sd(teamstats$opp.avg.point)),
                                  normalscorediff = pnorm(as.numeric(score.diff.avg), 
                                                          mean = mean(score.diff.avg), 
                                                          sd = sd(teamstats$score.diff.avg))) 

```

Merging team and opponent generated feature data into main dataset.
...
In the same way that it's extremely useful to combine team and opponent aggregate data into the main dataset, it's also very important to find out just how well the opponents' opponents stack up, so we will create the same type of features for opponents' opponents as we did in the steps above and merge that data into the main dataset. 
...
oppstatsagg <- oppstatsagg %>% mutate(normaloppaggavgpoint = pnorm(as.numeric(oppaggavgpoint), 
                                                                   mean = mean(oppaggavgpoint), 
                                                                   sd = sd(oppaggavgpoint)),
                                      normaloppaggscorediffavg = pnorm(oppaggscorediffavg), 
                                                                       mean = mean(oppaggscorediffavg), 
                                                                       sd = sd(oppaggscorediffavg),
                                      normalaggavgwinpct = pnorm(as.numeric(aggavgwinpct), 
                                                                 mean = mean(aggavgwinpct), 
                                                                 sd = sd(aggavgwinpct)))
...
Normalizing game data for home field advantage.  Due to a number of factors, home field advantage is a very difficult concept to quantify.  It differs from team to team and possibly even from opponent to opponent.  It fluctuates based on the ebb and flow of a game.  It's skewed because of the way scheduling tends to work for out of conference games; top teams will generally pay teams that are assumed to be significantly weaker than they are to come to their home stadium for what amounts to a glorified practice for these elite teams.  These results are often very lopsided, which skews the average margin of victory toward home teams.  Simply put, better teams tend to play more home games and win with larger margins, so determining home field advantage is a very biased task.  As such, I have decided to side with conventional bettor's wisdom and assume that home field advantage is worth approximately 2 points in any given game.  
...
Removing fields that are highly correlated with one another.
...
And creating two separate generalized linear models:  One to model one team's score, another to project the other team's score.  Generating predictions for the test set and determining the RMSE.  A generalized linear model was chosen for a few different reasons.  First, most of the features in the data are normally distributed, a property than lends itself well to linear modeling.  More importantly, after running multiple variations of random forest, neural networks, and boosted models, validation error with the GLM was no higher than any of the other more computationally complex models.  
...
In football, most scoring is either by way of a touchdown (which typically gives 7 points) or a field goal (for 3 points).  There are possibilities for 2, 6, and 8 point scores, but those are not likely to occur in most games.  As such, there are some point totals that are far more likely to occur than others.  While it is possible to score 4 points in a single game, the likelihood of that occuring is miniscule as it has only happened a few times in over 100 years.  It is far more probable that a team will score 3 points than 4.  The following code alters the unlikely scores predicted by each model to reflect more common point totals.
...
The models will periodically project a tie between two teams; however, because of overtime, no game will actually conclude with a tie.  Noted above, overtime scores tend to skew the scoring statistics in a highly variable way, so it's difficult to predict that.  However, because the algorithm initially generates an unrounded score (it may generate a score of 20.452313 to 19.892341) that gets rounded to 20 to 20, it does actually pick a winner so the winning score is tweaked to reflect a close game where the winner wins by one point.
...
Time to use the GLM model to predict the bowl games themselves.  Note that the schedule doesn't assume to know the winner of the semi-final playoff games, so all 4 potential final matchups are predicted.
...
names(bowl_prediction) <- c("team1", "team2", "team1score", "team2score")

print(bowl_prediction)

```

Now that the bowl games have all been played, we can compare the projected results to the actual results.  In how many games did the algorithm correctly project the winner, and how close was it overall?
...
This might not seem significantly better than a simple coin flip, but the bowl results from this year included several minor upsets.  As such, RMSE would likely be a better metric to determine how well the model fits the data rather than accuracy, but since we lack enough data to compare against, a RMSE evaluation is somewhat baseless. Additionally, most bowl games involve a different psychological mindset from regular season matchups that is difficult to quantify; some teams are very happy to get into the bowl game in which they are playing, while others are disappointed that their seasons didn't end better.  Additionally, there are other factors that go into projecting games between two teams in which there are few (if any) overlapping results to directly compare against.  
For comparison, an ESPN college football writer correctly projected the winners of 19 games. Link here:  
